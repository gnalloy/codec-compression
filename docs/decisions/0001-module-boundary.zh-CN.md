# ADR-001：保持 codec-compression 为独立 Gnalloy 模块
...
`codec-compression` 作为 `gnalloy.org/codec-compression` 发布，并拥有该职责：Gnalloy Pipeline 的压缩/解压编解码器，覆盖 gzip、zlib、brotli、snappy、lz4、zstd、bzip2、lzma、FastLZ 与 LZF。
