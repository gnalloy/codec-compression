...
import (
	...
	base "gnalloy.org/codec-compression"
...
func TestHandlersRoundTrip(t *testing.T) {
	...
	compressed := testutil.EncodeWithHandler(t, encoder, []byte("hello lz4"))
	decoded := testutil.DecodeWithHandler(t, decoder, compressed)
	if !bytes.Equal(decoded.Bytes(), []byte("hello lz4")) {
...
func TestEncoderRejectsInvalidLevel(t *testing.T) {
	_, err := NewEncoder(CompressionLevel(^uint32(0)))
